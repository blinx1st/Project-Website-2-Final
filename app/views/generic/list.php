<?php
// View danh sách dùng chung: cfg.list tạo cột, rows là dữ liệu và canWrite quyết định hiện action ghi.
$isPostList = (($data['cfg']['table'] ?? '') === 'BaiDang');
?>
    ...
    <?php if ($isPostList): ?>
        <div class="post-feed">
            <?php foreach ($data['rows'] as $row): ?>
                <?php
                // Chuẩn hóa thông tin tác giả/ngày và dựng đủ tham số khóa chính cho các action.
                    ...
                    <?php foreach ($data['rows'] as $row): ?>
                        <tr data-ma-sukien="<?= h($row['MaSuKien'] ?? '') ?>" data-ma-thanhvien="<?= h($row['MaThanhVien'] ?? '') ?>">
                            ...
                            <td style="white-space:nowrap;">
                                <?php
                                // Resource có thể dùng một khóa hoặc khóa kép nên URL được dựng động từ cfg.pk.
                                $params = [];
                                foreach ($data['cfg']['pk'] as $pk) {
                                    $params[$pk] = $row[$pk];
                                } ?>
